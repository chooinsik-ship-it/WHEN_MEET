...
export default function TimeGrid({ schedule, onChange, title }: TimeGridProps) {
  ...
  // 항상 최신값을 ref에 유지 (클로저 없이 native 이벤트에서 사용)
  ...
  /**
   * 모바일 터치 전용 native 이벤트 등록 (passive: false → preventDefault 가능)
  ...
  useEffect(() => {
    ...
    const onTouchStart = (e: TouchEvent) => {
      ...
      if (!cell) return; // 헤더/라벨 → 스크롤 허용

      e.preventDefault(); // 셀 터치 시 스크롤 차단
